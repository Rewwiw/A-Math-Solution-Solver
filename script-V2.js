...
function couldBeNum(t){ return mustBeNum(t) || t==='?'; }
function couldBeOp(t) { return mustBeOp(t)||t==='+/-'||t==='x/÷'||t==='x'||t==='÷'||t==='?'; }
...
function buildSegmentUI() {
  ...
  segments.forEach((seg, si) => {
    ...
    header.className = "seq-row-header";
    ...
    addBtn.textContent = "+";
    addBtn.title = "Add fixed tile slot";
    addBtn.addEventListener("click", () => {
      segments[si].tiles.push("");
      buildSegmentUI();
    });
    ...
    if (seg.tiles.length > 0) {
      ...
      remBtn.className = "remove-tile-btn";
      remBtn.innerHTML = "&minus;";
      remBtn.title = "Remove last tile slot";
...
function buildModalPad() {
  ...
  PAD_TILES.forEach(tile => {
    ...
    ptBadge.textContent = tilePoint(tile.v);
    wrap.appendChild(ptBadge);
    wrap.addEventListener("click", () => selectModalTile(tile.v));
...
function selectModalTile(val) {
  if (activeModalTarget && activeModalTarget._segIdx !== undefined) {
...
function clearSelectedTileSlot() {
  if (activeModalTarget && activeModalTarget._segIdx !== undefined) {
    const si = activeModalTarget._segIdx;
    const ti = activeModalTarget._tileIdx;
    segments[si].tiles[ti] = "";
    closeTileModal();
    buildSegmentUI();
    updatePreview();
  }
...
function removeSegment() {
  ...
  segments.pop();
  if (currentTarget && currentTarget._segIdx >= segments.length) selectTopTarget();
  buildSegmentUI();
}
...
async function runAMath() {
  ...
  if (mode==='mode1') {
    ...
    for (let wi = 0; wi < total; wi++) {
      ...
      const { k, subset } = workUnits[wi];
      for (const perm of permsGen(subset)) {
        if (ConditionTemplate(perm)) {
          for (const exp of expandGen(perm)) {
            if (Condition(exp) && Check_Equation(exp)) {
              const key = exp.join('');
              if (!seenKey.has(key)) {
                seenKey.add(key);
                const score = computeScore(subset, [], k);
                solutions.push({ eq: key, score, usedTiles: subset, usedCount: k });
              }
            }
          }
        }
      ...
      if (now - lastYield >= YIELD_MS) {
        setProgress(((wi + 1) / total) * 100, `Checking ${wi + 1}/${total} — ${solutions.length} found`);
        await yieldFrame();
